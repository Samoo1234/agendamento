...
async function enviarMensagemConfirmacao() {
  try {
    console.log('Enviando mensagem usando template de confirmação...');
    ...
    // Usar o template "confirmacao_agendamento" que já está aprovado
    const data = {
    ...
    fs.writeFileSync('ultima-requisicao-confirmacao.json', JSON.stringify({
      url,
      headers,
      data
    }, null, 2));
    ...
    fs.writeFileSync('ultima-resposta-confirmacao.json', JSON.stringify({
    ...
    console.error('❌ Erro ao enviar mensagem de confirmação:');
    if (error.response) {
      console.error('Status:', error.response.status);
      console.error('Dados:', JSON.stringify(error.response.data, null, 2));
      
      // Salvar o erro em um arquivo para referência
      fs.writeFileSync('ultimo-erro-confirmacao.json', JSON.stringify({
