/**
 * SERVIDOR LOCAL WHATSAPP - VERSÃO CORRIGIDA
 * 
 * Este servidor recebe webhooks do WhatsApp, processa as mensagens com a OpenAI
 * e envia as respostas de volta para o WhatsApp.
 */
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (!body.entry || !body.entry[0]) {
      registrarLog('ERROR', 'Entradas não encontradas na requisição');
      return;
    }
    
    // Verificar se há mudanças
    if (!body.entry[0].changes || !body.entry[0].changes[0]) {
      registrarLog('ERROR', 'Mudanças não encontradas na requisição');
      return;
    }
    
    // Verificar se há valor nas mudanças
    if (!body.entry[0].changes[0].value) {
      registrarLog('ERROR', 'Valor não encontrado nas mudanças');
      return;
    }
    ...
    if (!value.messages || !value.messages[0]) {
      registrarLog('INFO', 'Não há mensagens na requisição, pode ser uma notificação de status');
      return;
    }
    ...
    if (!value.contacts || !value.contacts[0]) {
      registrarLog('ERROR', 'Contatos não encontrados na requisição');
      return;
    }
    
    const telefone = value.contacts[0].wa_id;
